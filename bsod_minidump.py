...
Online WinDbg install uses CDN MSIX + Add-AppxPackage; ``winget show`` and winget-pkgs metadata
are lookup-only fallbacks — not ``winget install`` on the hot path (see ``_install_windbg_online``).
"""
...
from bsod_runtime import console_print, windbg_install_log_path
...
def _get_cdb_search_paths() -> list:
    """CDB search order: writable local copy, then online/offline policy, then SDK.

    Online: prefer WinDbg app (newest via winget/Store) over the bundled build.
    Offline: prefer bundled engine shipped with the portable app.
...
# Last-resort WinDbg MSIX CDN URL (matches winget-pkgs release; refresh via scripts/sync_install_fallbacks.py).
WINDDBG_MSIX_FALLBACK_VERSION = "1.2606.22001.0"
WINDDBG_MSIX_FALLBACK_URL = (
    "https://windbg.download.prss.microsoft.com/dbazure/prod/1-2606-22001-0/windbg.msixbundle"
)
...
CDB_INSTALL_EXPLANATION = """
  ...
  latest from Microsoft (direct MSIX download + Add-AppxPackage; no winget install step)."""

def _version_sort_key(version: str) -> tuple[int, ...]:
    """Compare winget-style versions (1.2606.22001.0) for staleness checks."""
    parts: list[int] = []
...
def _windbg_install_log(msg: str) -> None:
    line = f"[{datetime.now():%H:%M:%S}] {msg}\n"
    try:
        with open(windbg_install_log_path(), "a", encoding="utf-8") as fh:
            fh.write(line)
    except OSError:
        pass


def _reset_windbg_install_log() -> None:
    try:
        with open(windbg_install_log_path(), "w", encoding="utf-8") as fh:
            fh.write("")
    except OSError:
        pass
...
def _windbg_msix_url_from_github_manifest() -> str | None:
    """Latest WinDbg .msixbundle URL from public winget-pkgs (no winget.exe required)."""
    try:
        api = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/m/Microsoft/WinDbg"
        req = urllib.request.Request(api, headers={"User-Agent": "BSODAnalyzer"})
        ...
        yaml_url = (
            ...
            f"manifests/m/Microsoft/WinDbg/{ver}/Microsoft.WinDbg.installer.yaml"
        )
        req2 = urllib.request.Request(yaml_url, headers={"User-Agent": "BSODAnalyzer"})
        with urllib.request.urlopen(req2, timeout=45) as resp2:
            yaml_text = resp2.read().decode("utf-8", errors="ignore")
        m = re.search(r"InstallerUrl:\s*(\S+)", yaml_text, re.IGNORECASE)
        return m.group(1).strip() if m else None
    except (urllib.error.URLError, OSError, ValueError, KeyError, json.JSONDecodeError):
        return None


def _resolve_windbg_msix_url() -> str:
    """Best-effort latest WinDbg MSIX URL: winget show → GitHub manifest → shipped fallback."""
    return (
        _windbg_msix_download_url()
        or _windbg_msix_url_from_github_manifest()
        or WINDDBG_MSIX_FALLBACK_URL
    )


def _windbg_latest_version_label() -> str | None:
    """Human-readable newest WinDbg package version when metadata lookup succeeds."""
    ver = _winget_available_version(WINDBG_WINGET_PACKAGE)
    if ver:
        return ver
    try:
        api = "https://api.github.com/repos/microsoft/winget-pkgs/contents/manifests/m/Microsoft/WinDbg"
...
def _windbg_msix_download_url() -> str | None:
    """Latest WinDbg .msixbundle URL from winget metadata (Microsoft CDN)."""
    try:
        proc = subprocess.run(
            ["winget", "show", "--id", WINDBG_WINGET_PACKAGE, "-e",
             "--accept-source-agreements", "--disable-interactivity"],
            capture_output=True, text=True, timeout=90,
            creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0,
        )
        out = (proc.stdout or "") + (proc.stderr or "")
        m = re.search(r"^\s*Installer Url:\s*(\S+)\s*$", out, re.MULTILINE | re.IGNORECASE)
        return m.group(1).strip() if m else None
    except (subprocess.TimeoutExpired, FileNotFoundError, OSError):
        return None


def _windbg_engine_ready() -> bool:
    clear_cdb_path_cache()
    return _find_windbg_app_engine_dir() is not None or find_cdb() is not None


def _finalize_online_windbg_install(_say) -> tuple[bool, str] | None:
    if not _windbg_engine_ready():
        return None
    ...
    if find_cdb():
        return True, (
            "WinDbg/CDB installed. Run the tool once as Administrator to also save "
            "a local offline copy."
        )
    return True, "WinDbg/CDB installed."


def _install_windbg_msix_bundle(bundle_path: str) -> tuple[bool, str]:
    """Register WinDbg MSIX with Add-AppxPackage (bypasses winget ShellExecute install)."""
    if not bundle_path or not os.path.isfile(bundle_path):
        msg = f"WinDbg package not found: {bundle_path}"
        _windbg_install_log(msg)
        return False, msg
    _windbg_install_log(f"Registering WinDbg MSIX: {bundle_path}")
    path_ps = bundle_path.replace("'", "''")
    script = f"""
$ErrorActionPreference = 'Stop'
Add-AppxPackage -LiteralPath '{path_ps}' -ForceApplicationShutdown
"""
    try:
        proc = subprocess.run(
        ...
        _windbg_install_log(f"Add-AppxPackage error: {exc}")
        return False, str(exc)
    out = ((proc.stderr or "") + (proc.stdout or "")).strip()
    if out:
        _windbg_install_log(f"Add-AppxPackage output: {out[:2000]}")
    if proc.returncode == 0 or _windbg_engine_ready():
        _windbg_install_log("WinDbg MSIX registered successfully.")
        return True, "WinDbg MSIX installed."
    err = out or f"Add-AppxPackage exit code {proc.returncode}"
    _windbg_install_log(f"Add-AppxPackage failed: {err[:500]}")
    return False, err


def _download_windbg_msix_bundle(dest_path: str, url: str) -> tuple[bool, str]:
    """Download WinDbg .msixbundle from Microsoft CDN."""
    _windbg_install_log(f"Downloading WinDbg from {url}")
    url_ps = url.replace("'", "''")
    dest_ps = dest_path.replace("'", "''")
    script = f"""
$ErrorActionPreference = 'Stop'
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
Invoke-WebRequest -Uri '{url_ps}' -OutFile '{dest_ps}' -UseBasicParsing
"""
    try:
        proc = subprocess.run(
            ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", script],
        ...
        _windbg_install_log(f"Download error: {exc}")
        return False, str(exc)
    out = ((proc.stderr or "") + (proc.stdout or "")).strip()
    if out:
        _windbg_install_log(f"Download output: {out[:2000]}")
    if proc.returncode == 0 and os.path.isfile(dest_path):
        _windbg_install_log(f"Download complete: {dest_path}")
        return True, "Downloaded WinDbg from Microsoft."
    err = out or f"Download failed (exit {proc.returncode})."
    _windbg_install_log(f"Download failed: {err[:500]}")
    return False, err


def _install_windbg_online(_say) -> tuple[bool, str]:
    """Install or upgrade WinDbg: CDN download + Add-AppxPackage (never winget install)."""
    _reset_windbg_install_log()
    _windbg_install_log("BSOD Analyzer WinDbg install started.")
    _say("Installing latest WinDbg from Microsoft…")
    dest_dir = os.path.join(tempfile.gettempdir(), "BSODAnalyzer_windbg_dl")
    os.makedirs(dest_dir, exist_ok=True)
    dest = os.path.join(dest_dir, "windbg.msixbundle")
    url = _resolve_windbg_msix_url()
    _windbg_install_log(f"Resolved MSIX URL: {url}")
    ok, dl_msg = _download_windbg_msix_bundle(dest, url)
    if not ok:
        _windbg_install_log(f"Install aborted: {dl_msg}")
        return False, dl_msg or "WinDbg download failed."
    ok, msg = _install_windbg_msix_bundle(dest)
    if ok:
        _windbg_install_log("WinDbg install complete.")
        return True, msg
    _windbg_install_log(f"Install failed: {msg}")
    return False, msg
...
def install_cdb(progress=None) -> tuple[bool, str]:
    ...
    Online: installs the Microsoft WinDbg app via direct MSIX download + Add-AppxPackage.
    ...
    existing = find_cdb()
    needs_windbg_app = _is_online() and _find_windbg_app_engine_dir() is None
    if existing and not needs_windbg_app:
        return True, f"CDB is already available ({existing})."

    if _is_online():
        ok, err = _install_windbg_online(_say)
        finished = _finalize_online_windbg_install(_say)
        if finished:
            return finished
        return False, (
            "WinDbg install did not complete. Try installing WinDbg from the Microsoft Store, "
            "then reopen this tool.\n\n" + (err or "")[:500]
            + f"\n\nDetails: {windbg_install_log_path()}"
        )
...
def update_cdb(progress=None) -> tuple[bool, str]:
    ...
    latest_pkg = _windbg_latest_version_label()
    if latest_pkg:
        _say(f"Installed CDB version: {before_ver}. Newest WinDbg from Microsoft: {latest_pkg}.")
    else:
        _say(f"Installed CDB version: {before_ver}. Checking Microsoft for a newer WinDbg build…")
    newest_note = f" Newest WinDbg available: {latest_pkg}." if latest_pkg else ""
    ok, err = _install_windbg_online(_say)
    if not ok and not _windbg_engine_ready():
        return False, err or "WinDbg update did not complete."
...
def should_prompt_cdb_online_install(settings: dict | None) -> bool:
    """True when online and the WinDbg app is not installed (bundled/offline fallback only)."""
    if sys.platform != "win32":
        return False
    if not _is_online():
        return False
    s = settings or {}
    if s.get("cdb_online_install_dismissed"):
        return False
    if s.get("cdb_online_install_prompt", True) is False:
        return False
    return _find_windbg_app_engine_dir() is None


def prompt_install_cdb() -> bool:
    """CLI: explain why CDB helps, then install (online: direct MSIX; offline: bundled SDK)."""
    try:
    ...
    try:
        ...
        for line in output.splitlines():
            ...
            if "MODULE_NAME:" in s:
                ...
                if frame and len(parsed["stack_frames"]) < _MAX_STACK_FRAMES:
                    parsed["stack_frames"].append(frame)
            elif s.startswith("# ") and "kb" not in s.lower() and len(parsed["stack_frames"]) < _MAX_STACK_FRAMES:
                parts = s.split(None, 2)
                if len(parts) >= 3 and "!" in parts[2]:
                    parsed["stack_frames"].append(parts[2].strip())
        if stack_text_lines and not parsed["stack_frames"]:
            for st in stack_text_lines[:_MAX_STACK_FRAMES]:
                if "!" in st:
...
_KERNEL_STACK_MODULES = frozenset({
    "nt", "ntkrnlmp", "ntkrnlpa", "ntoskrnl", "hal",
    "win32k", "win32kbase", "win32kfull", "dxgkrnl", "dxgmms2",
    "watchdog", "fltmgr", "ndis", "netio", "tcpip", "afd",
    "ci", "cng", "msrpc", "clipsp", "volmgr", "volmgrx", "partmgr", "disk", "ntfs",
})


def _stack_module_base(frame: str) -> str:
    mod = frame.split("!", 1)[0].lower().strip()
    return mod.replace(".sys", "").replace(".dll", "").replace(".exe", "")


def first_actionable_stack_frame(stack_frames: list[str] | None) -> str | None:
    """First stack frame that is not a known kernel/shim module (best driver lead)."""
    for frame in stack_frames or []:
        if "!" not in frame:
            continue
        base = _stack_module_base(frame)
        if not base:
            continue
        if base in _KERNEL_MODULE_ALIASES:
            continue
        if base in _KERNEL_STACK_MODULES:
            continue
        return frame
    return None
...
def enrich_windbg_analysis(parsed: dict) -> dict:
    ...
    frames = parsed.get("stack_frames") or []
    if frames:
        hint = _kernel_only_stack_top(frames)
        if hint:
            parsed["kernel_stack_top"] = hint
        lead = first_actionable_stack_frame(frames)
        if lead:
            parsed["actionable_stack_frame"] = lead
    return parsed
